...
import { PaymentMethodTag, PaymentMethodTiles, type MethodTotals } from "@/components/PaymentMethodBreakdown";
...
function RelatedDetail({ row }: { row: BillHistoryRow }) {
  ...
  return (
    <div className="grid gap-4 border-t border-border/60 bg-muted/30 p-4 text-sm">
      <div>
        <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">
          Sales order
        </p>
...
/**
 * Bill history list. Used both as a standalone full page and as the
 * condensed left pane of the bills split view — `selectedId` highlights the
 * row currently open in the detail pane.
 */
export function BillsList({ selectedId }: { selectedId?: string }) {
  const navigate = useNavigate();
  const search = useSearch({ strict: false }) as { pending?: boolean };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Every bill with payments, warehouses, returns and credits.
          </p>
        </div>
      ...
      <div className="surface-card overflow-hidden">
        {/* Search + filter toggle — pinned above the scrolling rows */}
        <div className="sticky top-0 z-10 space-y-0 bg-card">
          <div className="flex flex-col gap-3 border-b border-border p-4 sm:flex-row">
            ...
            <Button variant="outline" className="h-11" onClick={() => setFiltersOpen((v) => !v)}>
              <SlidersHorizontal />
              Filters
            </Button>
          </div>
        ...
        {isLoading ? (
          ...
          <>
            {/* Condensed card/row style — used for both the split-pane list column and full mobile screens. */}
            <div className="divide-y divide-border/60">
              {pageRows.map((b) => (
                <div key={b.id} className={cn(b.id === selectedId && "bg-muted/60")}>
                  <Link
                    to="/bills/$billId"
                    params={{ billId: b.id }}
                    className="block space-y-2 p-4 hover:bg-muted/40 active:bg-muted/60"
                  >
                  ...
                  {expanded === b.id && (
                    <>
                      ...
                      <div className="flex gap-2 p-4 pt-3">
                        <Button asChild variant="outline" size="sm">
                          <Link to="/bills/$billId" params={{ billId: b.id }}>
