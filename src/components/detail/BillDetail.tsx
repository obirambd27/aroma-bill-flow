...
import { buildPaymentBreakdown, useBillAllocations, type PaymentLine } from "@/lib/bill-payments";

type PrintView = "a4" | "thermal";
/** Session-level memory of the last chosen print view. */
let lastPrintView: PrintView = "a4";

/** Collapsible itemized list of every payment event applied to this bill. */
function PaymentsReceivedSection({
  billId,
  lines,
  isError,
}: {
            ...
            <tbody>
              {lines.map((l) => (
                <tr key={l.key} className="border-b border-border/60 last:border-0">
                  ...
                  <td className="numeric px-4 py-2 text-right text-sm font-medium">
                    {formatMoney(l.amount)}
                  </td>
                  <td className="px-4 py-2 text-sm">{l.method}</td>
                  <td className="px-4 py-2 text-sm text-muted-foreground">
                    {l.accountName ?? (l.source === "billing" ? "At billing" : "—")}
                  </td>
                  <td className="px-4 py-2 text-sm">
                    {l.source === "payment" ? (
                      ...
                      >
                        {l.reference ?? "View payment"}
                      </Link>
                    ) : (
                      <span className="text-muted-foreground">
                        {l.reference ?? `Bill ${billId ? "" : ""}—`.trim()}
                      </span>
                    )}
                  </td>
                </tr>
...
export function BillDetail({ billId }: { billId: string }) {
  ...
  if (isError) {
    return (
      <DetailError message="This bill could not be loaded." onRetry={() => void refetch()} />
    );
  }
  ...
  return (
    <div className="space-y-5">
      <div className="no-print space-y-4">
        ...
        <PaymentsReceivedSection billId={bill.id} lines={breakdown.lines} isError={allocationsError} />
        {allocationsError && (
          <div className="flex justify-end">
            <Button variant="ghost" size="sm" onClick={() => void refetchAllocations()}>
