...
export function PurchaseBillDetail({ purchaseBillId }: { purchaseBillId: string }) {
  const navigate = useNavigate();
  const queryClient = useQueryClient();
  const { data: bill, isLoading, isError, refetch } = usePurchaseBill(purchaseBillId);
  ...
  if (isError || !bill) {
    return (
      <DetailError
        message="This purchase bill could not be found."
